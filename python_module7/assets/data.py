...
class CardGenerator:
    ...
    def get_cards_by_cost(
        ...
        Returns:
            Dictionary with card types as keys and
            lists of matching cards as values
        """
        result = {"creatures": [], "spells": [], "artifacts": []}
    ...
    def list_available_cards(self) -> None:
        ...
        print("\nSpells:")
        for spell in self._spells:
            print(f"  - {spell['name']} ({spell['cost']}"
                  f" mana, {spell['rarity']})")

        print("\nArtifacts:")
        for artifact in self._artifacts:
            print(f"  - {artifact['name']} ({artifact['cost']}"
                  f" mana, {artifact['rarity']})")
